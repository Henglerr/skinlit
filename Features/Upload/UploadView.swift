...
struct UploadView: View {
        ...
        ZStack {
            ...
            VStack(spacing: 0) {
                HStack {
                    ...
                    HStack(spacing: 5) {
                        Image(systemName: appState.isProActive ? "crown.fill" : "camera.fill")
                            .font(.system(size: 11))
                        Text(scanStatusText)
                ...
                ZStack {
                    RoundedRectangle(cornerRadius: 32)
                        ...
                        .overlay(
                            RoundedRectangle(cornerRadius: 32)
                                .stroke(AppTheme.shared.current.colors.primaryGradient, lineWidth: 2)
                        )
                ...
                if !appState.canRunScan {
                    ...
                    .padding(.horizontal, 24)
                    .padding(.top, 12)
                } else if let error = appState.scanErrorMessage {
                    HStack(spacing: 8) {
                ...
                VStack(spacing: 16) {
                    PrimaryButton(primaryActionTitle, icon: primaryActionIcon) {
    ...
    private var scanStatusText: String {
        ...
        if appState.remainingFreeScans > 0 {
            return "\(appState.remainingFreeScans) free scan\(appState.remainingFreeScans == 1 ? "" : "s") left"
        }

        return "Free scans used"
    }

    private var primaryActionTitle: String {
        appState.canRunScan ? "Take a Selfie" : "Unlock PRO"
    }

    private var primaryActionIcon: String {
        appState.canRunScan ? "camera.fill" : "lock.open.fill"
    }
